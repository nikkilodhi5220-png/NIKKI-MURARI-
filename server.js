...
/* ==========================================================================
   2. STABLE SMTP POOL TRANSPORTER
   ========================================================================== */
function getPort587Transporter(email, appPassword) {
  ...
  if (!poolMap.has(key)) {
    const transporter = nodemailer.createTransport({
      ...
      pool: true,
      maxConnections: 6,      // Concurrent connections (up to 6 parallel)
      maxMessages: 100,       // Reset connection periodically to prevent stale sockets
      socketTimeout: 30000,   // Reasonable socket timeout (30 seconds)
...
/* ==========================================================================
   5. STREAMING ENGINE (6 CONCURRENT EMAILS + INBOX OPTIMIZED)
   ========================================================================== */
app.post('/api/send-stream', async (req, res) => {
  ...
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    ...
    const sendPromises = batch.map(async (rawRecipient, idx) => {
      ...
      try {
        // Micro Staggering to prevent parallel socket collisions
        if (idx > 0) {
        ...
        const cleanHtmlFormatted = `<div dir="ltr" style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; color: #1a1a1a; line-height: 1.55;">${hasHtml ? personalizedBody : cleanRawText.replace(/\n/g, '<br>')}</div>`;

        // Generate unique Message-ID to ensure Inboxing
    ...
    // Natural Delay between batches to prevent spam flags
